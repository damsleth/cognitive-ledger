...
def render(series: list[dict], metric: dict, charts_dir: Path, root: Path) -> Path:
    try:
        import matplotlib.pyplot as plt
        import numpy as np
    except ImportError:
        raise SystemExit("ledger ab charts requires matplotlib and numpy: pip install matplotlib numpy")
...
def main(root: Path | None = None) -> None:
    ...
    if not series_path.is_file():
        raise SystemExit(f"performance_series.json not found at {series_path}")
    series = json.loads(series_path.read_text())
    series.sort(key=lambda r: r["generated_at"])
    charts_dir.mkdir(parents=True, exist_ok=True)
    for metric in METRICS:
        out = render(series, metric, charts_dir, root)
