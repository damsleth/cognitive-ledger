...
The semantic_rerank mode runs `semantic_hybrid` first to get an input_k
candidate pool, then uses a cross-encoder to score each (query, document)
pair and re-sorts by cross-encoder score.
...
def get_reranker(model_name: str, max_length: int = 512) -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            "semantic_rerank requires sentence-transformers. "
