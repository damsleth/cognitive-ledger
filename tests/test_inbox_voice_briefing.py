...
class InboxModuleTests(unittest.TestCase):
    ...
    def test_cleanup_inbox_returns_empty_when_dir_missing(self):
        ...
        shutil.rmtree(self.cfg.ledger_notes_dir / "00_inbox")
        from ledger.inbox import cleanup_inbox
        # "unheld_locks" sweeps the whole notes tree, so a missing 00_inbox no
        # longer means there is nothing to report — assert per key.
        result = cleanup_inbox()
        self.assertEqual(result["orphaned_locks"], [])
        self.assertEqual(result["stale_items"], [])
        self.assertEqual(result["logged_rejections"], [])
        self.assertIn("unheld_locks", result)
