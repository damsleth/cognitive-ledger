...
## Status 2026-09-25 (reconciled against yaams)
...
- **Blocked: PR 4 needs a human sign-off on the abbreviation gold set** (corrected 2026-09-25). The labels already exist: `~/brain/feed/eval/promotion_abbrev_gold.csv`, 130 rows, `label_source` agent-judged, 7 rows `needs_review=1`, pinned by hash in yaams `scripts/abbrev_gold_manifest.json`. The empty file is `promotion_abbrev_worksheet.csv`, which is only the unlabelled input. The gap is a human pass, not labelling from scratch: resolve the 7 flagged rows, spot-check the rest, set `label_source` to human for rows confirmed, then re-pin the manifest hash. A model-only gold set would be the synthetic-verdict-as-truth confusion this plan forbids.
...
- [x] PR 2 — abbreviation discovery: mine explicit patterns from the frozen fixture, label the 130-row worksheet into the first gold set; no ledger writes. Also define the Phase 0 train/dev/holdout splits before any proposer evaluation. _(Done 2026-08-31, yaams 22e6b6f: `scripts/promotion_splits.py` — deterministic thread-grouped 80/10/10 hash splits (74,706/5,678/7,334), pinned in `promotion_splits.json` with `--check`; `scripts/abbrev_mine.py` — explicit-pattern mining over non-holdout items only (428 candidates → `~/brain/feed/eval/promotion_abbrev_mined.jsonl`), heuristic pre-label + agent-reviewed gold set `~/brain/feed/eval/promotion_abbrev_gold.csv` (130 rows: 60 unknown/negative, 20 nickname, 13 initialism, 11 shorthand, 9 identifier, 9 abbreviation, 4 acronym, 4 code) pinned by hash in `abbrev_gold_manifest.json`. Open: 7 rows flagged needs_review await a human skim; labels are agent-judged, spot-check before treating precision floors as gospel.)_
- [x] PR 3 — run state: YAAMS `promotion_runs` migration, candidate state fields, export command and idempotency tests. _(Done: yaams `dc81c9c`, see Status.)_
- [ ] **Owner:** skim `~/brain/feed/eval/promotion_abbrev_gold.csv` (7 `needs_review` rows first), confirm or fix labels, re-pin `abbrev_gold_manifest.json`.
- [ ] PR 4 — abbreviation tracer bullet: evidence-bound abbreviation proposals, ambiguity handling, shadow facts and abbreviation retrieval/answer cases. Starts once the gold set is human-signed.
