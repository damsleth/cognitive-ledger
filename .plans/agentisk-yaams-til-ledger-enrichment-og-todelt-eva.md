...
- [x] PR 2 — abbreviation discovery: mine explicit patterns from the frozen fixture, label the 130-row worksheet into the first gold set; no ledger writes. Also define the Phase 0 train/dev/holdout splits before any proposer evaluation. _(Done 2026-08-31, yaams 22e6b6f: `scripts/promotion_splits.py` — deterministic thread-grouped 80/10/10 hash splits (74,706/5,678/7,334), pinned in `promotion_splits.json` with `--check`; `scripts/abbrev_mine.py` — explicit-pattern mining over non-holdout items only (428 candidates → `~/brain/promotion_abbrev_mined.jsonl`), heuristic pre-label + agent-reviewed gold set `~/brain/promotion_abbrev_gold.csv` (130 rows: 60 unknown/negative, 20 nickname, 13 initialism, 11 shorthand, 9 identifier, 9 abbreviation, 4 acronym, 4 code) pinned by hash in `abbrev_gold_manifest.json`. Open: 7 rows flagged needs_review await a human skim; labels are agent-judged, spot-check before treating precision floors as gospel.)_
- [ ] PR 3 — run state: YAAMS `promotion_runs` migration, candidate state fields, export command and idempotency tests.
