...
- [ ] **Bridge signal scoring to Ebbinghaus reinforcement.** — BLOCKED 2026-09-23: 1 retrieval_hit / 0 preference_applied live; nothing to ramp on. Treat `retrieval_hit` / `preference_applied` signals as R = e^(-t/S) events (S incremented on recall, dt reset) that bump a per-note `strength`, replacing the hard 20-signal binary gate with a continuous ramp. Cap the boost (reuse `validation_boost_cap: 0.15`); keep synthetic signals down-weighted. Source: report B3b + MemoryBank (arXiv 2305.10250).
